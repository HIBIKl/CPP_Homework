    ...
    : QWidget(parent)
{
	...
	btn2->show();
	//下拉复选框如何实现？if？需要对接
	connect(this->btn1, SIGNAL(QPushButton::clicked), this,SLOT(startAdjust(char** argv)));
	//分别输入1.原灰度图Mat类 ？ 2.转灰度图文件名？？？
	//3.解码后的文件名？？？？？ 4.EPSILON（变量名为xgm 类型为int）
	//I really need to figure out this one!!!!!!!!!	argv
...
void picture::startAdjustS(char**)
{
	int reg_num = 0;
	int bitnum = 0;
	double thresU = 0;
